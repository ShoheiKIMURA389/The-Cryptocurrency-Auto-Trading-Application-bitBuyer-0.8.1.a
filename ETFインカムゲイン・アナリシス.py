...
# 注意事項
# - このプログラムは教育目的及び参考用に提供されています。実際の投資判断には充分な調査と専門家への相談を行ってください。
# - 税率や投資条件は 2025 年時点の日本の制度を基にしています。将来の変更に対応するにはプログラムの修正が必要です。

# 取引手数料について
# このプログラムでは DMM 株を前提としており、売買手数料の計算には以下の考え方を採用しています。
# 1. 米国株の売買手数料
#    - DMM 株では、米国株及び米国 ETF の売買手数料として、約定金額の 0.495％ が発生します。
#    - この手数料は外貨決済時には直接約定金額に 0.495％ を乗じた金額として、円貨決済時には約定時の円レートに換算された金額として適用されます。
#    - このプログラムでは全て円で計算しているため、この売買手数料を「為替手数料」として処理しています。
# 2. 米国株以外の手数料
#    - DMM 株では、日本株や他の海外株式に関しては所定の売買手数料が発生します。
#    - しかし、このプログラムでは米国株 ETF の運用を想定しているため、それらの手数料は考慮していません。
# 3. 売却手数料について
#    - このプログラムは ETF の買付シミュレーションのみを行うため、売却に関する手数料計算は含んでいません。
#    - 売却ロジックを追加する場合、手数料の計算も追加する必要があります。

# 想定される利用ケース
# 1. ETF 投資の長期的なパフォーマンスの予測:
#    - 毎月配当型の特定の ETF（例: JEPI、HNDL、QYLD、XYLD）の配当金再投資戦略の効果を確認。
# 2. NISA 枠の最適利用:
...
為替手数料率 = 0.495  # 円から外貨への為替手数料（百分率％）
為替手数料上限 = 3300  # 為替手数料の上限金額（円、上限なし：0）
シミュレーション年数 = 20  # シミュレーション年数
...
def シミュレーションを実行する():
    ...
    保有株数 = ETF初期株数
    現在のETF単価 = ETF単価
    総投資額 = ETF初期株数 * 現在のETF単価  # 初期の総投資額
    繰越金額 = 0  # 月次繰越金額
    外貨率 = 1 - 為替手数料率 / 100  # 円から外貨への実効両替率
    ...
    def 為替手数料を計算する(買付株数):
        ...
        買付株数に基づき為替手数料を計算する。
        """
        為替手数料 = 買付株数 * 現在のETF単価 * 為替手数料率

        if 為替手数料上限 > 0:
            為替手数料 = 為替手数料 if 為替手数料上限 > 為替手数料 else 為替手数料上限

        return 為替手数料  # 為替手数料を返す
    ...
    for 年次 in range(1, シミュレーション年数 + 1):
        ...
        for 月 in range(1, 13):  # 月次イテレーション
            ...
            # 米国源泉徴収税率を適用して税引き前配当金を計算
            if 米国株:
                月次配当金 *= (1 - 米国源泉徴収税率 / 100)
            ...
            再投資可能金額 = 増資額 + 再投資額 + 繰越金額

            # 買付株数を計算（再投資可能金額が 0 以下の場合は買付なし）
            if 再投資可能金額 > 0:
                買付株数 = 再投資可能金額 // 現在のETF単価
                為替手数料 = 為替手数料を計算する(買付株数)

                while 買付株数 > 0 and (買付株数 * 現在のETF単価) + 為替手数料 > 再投資可能金額:
                    買付株数 -= 1
                    為替手数料 = 為替手数料を計算する(買付株数)

                繰越金額 = 再投資可能金額 - ((買付株数 * 現在のETF単価) + 為替手数料)
            else:
                買付株数 = 0
                繰越金額 = 再投資可能金額
